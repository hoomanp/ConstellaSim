...
# C-2: cap incoming JSON body size to 1 KB.
...
def set_security_headers(response):
    ...
    # MEDIUM-5: apply nonce to style-src, removing 'unsafe-inline' entirely.
...
def run_sim():
    ...
    if not dest_lat:
        # MEDIUM-1: return a generic message — reflecting the user-supplied city string
        # back in the error body acts as an oracle for sanitization bypass attempts.
        return jsonify({"error": "Destination could not be resolved"}), 400
